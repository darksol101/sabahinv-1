...
class MY_Model extends CI_Model {
	...
	public function save_call($db_array, $id=NULL, $district,$set_flashdata = TRUE) {
		...
		if ($id) {
			...
			//die('here');
			$this->db->set('call_uid',"CONCAT((SELECT district_code FROM sst_districts WHERE district_id =$district),DATE_FORMAT(CURDATE(),'%y%m%d'),(LPAD((SELECT COUNT(*)+1 FROM sst_calls AS c WHERE date(c.call_created_ts) <= date(NOW()) AND date(c.call_created_ts) > date(SUBDATE(NOW(),1)) ),3,'0')))",false);
				
			$success = $this->db->insert($this->table_name, $db_array,false);
	...
	public function check_if_exist($field,$value){
