...
class nepalicalendar{
	...
	function make_npcalendar($year, $month, $start_day, $day_name_length, $link = '', $month_name, $month_date, $num_day, $cal, $links)
	{
		$current_year = date('Y');
		$current_month = date('m');
		$current_day = date('d');
		
		$current_date = $this->eng_to_nep($current_year, $current_month, $current_day);
		$current_year = $current_date['year'];
		$current_month = $current_date['month'];
		$current_day = $current_date['date'];
		$table = '';
		$table =  '<table class="mod_npcal_table" align="center" border="0">'."\n";
		
		$year_options = array();
		for($yr=2000;$yr<=2090;$yr++){
			$year_options[$yr] = $yr;
		}
		$js= 'onchange="setcalendarurl('.$current_year.','.$current_month.');"';
		$year_dropdown = form_dropdown('year',$year_options,$year,'id="year" '.$js);
		$month_options = array(
								'1'=>'Baishak',
								'2'=>'Jestha',
								'3'=>'Ashad',
		...
		$month_dropdown = form_dropdown('month',$month_options,$month,'id="month" '.$js);
		$table.='<tr><td colspan="7"><div class="cal_title"><span>'.$month_dropdown.'</span><span>'.$year_dropdown.'</span></div></td></tr>';
		$table.=  '<tr>';
		
		// draw the month and year heading in the current language
		$month_time = mktime(0,0,0,$month,1,$year);
		
		$table.=  '<th colspan="7" align="center" class="mod_npcal_header">';
		if($links){
			$table.=  '<a href="'.$link.'p">&laquo;</a>&nbsp;&nbsp;';
			
			$table.=  $month_name . " " . $year;
		}
		if($links)
			$table.=  '&nbsp;&nbsp;<a href="'.$link.'n">&raquo;</a>';
		$table.=  '</th>';
		$table.=  '</tr>';
		...
		if ($day_name_length > 0)
		{
			...
			for ($i = 0; $i < 7; $i++)
			{
				...
				$day_name = gmstrftime('%A',$day_time);	// use GMT to get the day names
				if (function_exists('mb_substr'))
					$day_short_name = mb_substr($day_name,0,2,'UTF-8');	// prefer this if available
				else
					$day_short_name = substr($day_name,1,$day_name_length);		// use this if no mbstring library
				$table.=  '<th class="mod_npcal_dayname">'.utf8_encode($day_short_name).'</th>';
			}
			$table.=  '</tr>';
		}
			
		$bar = array(0=>'Saturday', 1=>'Sunday', 2=>'Monday', 3=>'Tuesday', 4=>'Wednesday', 5=>'Thursday', 6=>'Friday');
	
		//Find the first day
		$first_date_value = $month_date%7;  //if today's date is 16 and wednesday, then 2 is also wednesday
		$first_date_value = $first_date_value-1;
		$array_value = $num_day-$first_date_value;
		if($array_value < 0)
			$array_value = 7+$array_value;
			
		$first_weekday = $array_value;
		$first_weekday_name = @$bar[$array_value];
		$first_column = ($first_weekday + 7 - $start_day) % 7;
		if($first_column == 0)
			$first_column = 7;
		...
		if ($first_column > 0)
			$table.=  '<td class="split'.$first_column.'" colspan="'.$first_column.'">&nbsp;</td>'; // draw blank columns
		...
		for ($day = 1; $day <= $days_in_month; $day++)
		{
			...
			{
				$table.=  '<td onclick="convertdate('."'".$year."'".','."'".$month."'".','."'".$day."'".')" class="mod_npcal_today" id="'.$year.'_'.$month.'_'.$day.'"><a class="cal_today">'.$day.'</a></td>';	// highlight today's date
			}
			else
				$table.=   '<td onclick="convertdate('."'".$year."'".','."'".$month."'".','."'".$day."'".')" align="center" id="'.$year.'_'.$month.'_'.$day.'"><a>'.$day.'</a></td>';
			$column_count ++;
		}
		if ($column_count < 7)
			$table.=  '<td class="split'.(7 - $column_count).'" colspan="'.(7 - $column_count).'">&nbsp;</td>'; // draw blank columns
		$table.=  "</tr></table>\n";
		return $table;
}
}
/* Location: ./system/libraries/nepalicandar.php */