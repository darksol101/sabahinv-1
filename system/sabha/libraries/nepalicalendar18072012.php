...
class nepalicalendar{
	...
	function make_npcalendar($year, $month, $start_day, $day_name_length, $link = '', $month_name, $month_date, $num_day, $cal, $links)
	{
		...
		$table.='<tr><td colspan="4">'.$month_dropdown.'</td><td colspan="3">'.$year_dropdown.'</td></tr>';
		$table.=  '<tr>';
		...
		for ($day = 1; $day <= $days_in_month; $day++)
		{
			...
			{
				$table.=  '<td class="mod_npcal_today" id="'.$year.'_'.$month.'_'.$day.'">'.$day.'</td>';	// highlight today's date
			}
			else
				$table.=   '<td align="center" id="'.$year.'_'.$month.'_'.$day.'">'.$day.'</td>';
			$column_count ++;
		}
		if ($column_count < 7)
