...
class Entry extends Account_Controller {
	...
	function show($entry_type)
	{
		...
		if ($entry_type == "tag") {
			$this->db->from('entries')->where('reference_id',0)->where('tag_id', $tag_id)->where('sc_id', $this->session->userdata('sc_id'))->order_by('date', 'desc')->order_by('number', 'desc')->limit($pagination_counter, $page_count);
			$entry_q = $this->db->get();
			$config['total_rows'] = $this->db->from('entries')->where('reference_id >1')->where('tag_id', $tag_id)->where('sc_id', $this->session->userdata('sc_id'))->get()->num_rows();
		} else if ($entry_type_id > 0) {
			$this->db->from('entries')->where('reference_id',0)->where('entry_type', $entry_type_id)->where('sc_id', $this->session->userdata('sc_id'))->order_by('date', 'desc')->order_by('number', 'desc')->limit($pagination_counter, $page_count);
			$entry_q = $this->db->get();
			$config['total_rows'] = $this->db->from('entries')->where('reference_id >1')->where('entry_type', $entry_type_id)->where('sc_id', $this->session->userdata('sc_id'))->get()->num_rows();
		} else {
			$this->db->from('entries')->where('reference_id',0)->order_by('date', 'desc')->where('sc_id', $this->session->userdata('sc_id'))->order_by('number', 'desc')->limit($pagination_counter, $page_count);
			$entry_q = $this->db->get();
			$config['total_rows'] = $this->db->from('entries')->where('reference_id',0)->order_by('date', 'desc')->where('sc_id', $this->session->userdata('sc_id'))->num_rows();
		}
	...
	function view($entry_type, $entry_id = 0)
	{
		...
		$data['sc_code'] = $cur_entry->sc_code;
	...
	function add($entry_type)
	{
		...
		$this->db->select('sc_code');
		$this->db->from('sst_service_centers');
		$this->db->where('sc_id',$this->session->userdata('sc_id'));
		$sc = $this->db->get();
		if($sc->num_rows()>0){
			if($sc->row()->sc_code){
				$sc_code = $sc->row()->sc_code;
			}else{
				$this->session->set_flashdata('custom_error','Invalid service center code.');
				redirect('account');				
			}
		}else{
			$this->session->set_flashdata('custom_error','Invalid service center code.');
		...
		else
		{
			...
			$insert_data = array(
				...
				'entry_created_by'=>$this->session->userdata('user_id'),
				'entry_created_ts'=>date('Y-m-d H:i:s')
			);
			$this->db->set('sc_code','(SELECT sc_code FROM sst_service_centers WHERE sc_id='.$this->session->userdata('sc_id').')',false);
			...
			foreach ($data_all_ledger_dc as $id => $ledger_data)
			{
				...
				if ( ! $this->db->insert('entry_items', $insert_ledger_data))
				{
					...
					$entry_item_id = $this->db->insert_id();
					$sub_ledger = $this->input->post('sub_ledger_id');
					$sub_amount = $this->input->post('sub_amount');
					if(isset($sub_ledger[$id])){
						if(is_array($sub_ledger[$id])){
							$s=0;
							foreach($sub_ledger[$id] as $sub_ledger_id){
								$insert_sub_ledger_data['sub_ledger_id'] = $sub_ledger_id;
								$insert_sub_ledger_data['amount'] = $sub_amount[$id][$s];
								$insert_sub_ledger_data['dc'] = $data_ledger_dc;
								$insert_sub_ledger_data['entry_item_id'] = $entry_item_id;
								
								if ( ! $this->db->insert('sub_entry_items', $insert_sub_ledger_data)){
									$this->db->trans_rollback();
									$this->load->view('account/entry/add', $data);
									return;
								}else{									
	...
	{
		$this->load->model(array('account/mdl_sub_ledger'));
		$this->load->language("account",  $this->mdl_mcb_data->setting('default_language'));	   			
		/* Check access */
		...
		if ( ! $_POST)
		{
			$this->db->select ('entry_items.*,ledgers.has_sub_ledger');
			$this->db->from('entry_items')
			->join('ledgers','ledgers.id=entry_items.ledger_id','left')
			...
			foreach ($cur_ledgers_q->result() as $row)
			{
				...
				$data['entry_item_id'][$counter] = $row->id;
				$data['has_sub_ledger'][$counter] = $row->has_sub_ledger;					
				if ($row->dc == "D")
				{
			...
			$data['dr_amount'][$counter] = "";
			$data['cr_amount'][$counter] = "";
			$data['entry_item_id'][$counter] = 0;
			$data['has_sub_ledger'][$counter] = 0;
			$counter++;
			$data['ledger_dc'][$counter] = 'D';
			$data['ledger_id'][$counter] = 0;
			$data['dr_amount'][$counter] = "";
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$update_data = array(
				...
				'tag_id' => $data_tag,
				'sc_id' =>$sc_id,
				'entry_modified_by'=>$this->session->userdata('user_id'),
				'entry_modified_ts'=>date('Y-m-d H:i:s')
			);
			if ( ! $this->db->where('id', $entry_id)->update('entries', $update_data))
			{
				$this->db->trans_rollback();
			...
			foreach ($data_all_ledger_dc as $id => $ledger_data)
			{
				...
				if ( ! $this->db->insert('entry_items', $insert_ledger_data))
				{
					...
					$entry_item_id = $this->db->insert_id();
					$sub_entry_item_id = $this->input->post('sub_entry_item_id');
					...
					if(isset($sub_ledger[$id])){
						if(is_array($sub_ledger[$id])){
							$s=0;
							foreach($sub_ledger[$id] as $sub_ledger_id){
								$update_sub_ledger_data['sub_ledger_id'] = $sub_ledger_id;
								$update_sub_ledger_data['amount'] = $sub_amount[$id][$s];
								$update_sub_ledger_data['dc'] = $data_ledger_dc;
								$update_sub_ledger_data['entry_item_id'] = $entry_item_id;
								
								if($sub_entry_item_id[$id][$s] == 0){
									$success =  $this->db->insert('sub_entry_items', $update_sub_ledger_data);					
								}else{
									$success =  $this->db->where('sub_entry_item_id', $sub_entry_item_id[$id][$s])->update('sub_entry_items', $update_sub_ledger_data);
								}
								if (!$success){
									$this->db->trans_rollback();
									$this->load->view('account/entry/add', $data);
	...
	{
		$tabindex = $this->input->get('tabindex');
		...
		echo '<td>';
		echo form_dropdown_dc('ledger_dc[' . $i . ']','','tabindex="'.$tabindex.'"');
		$tabindex++;
		echo '</td>';

		echo '<td>';
		if ($add_type == 'bankcash')
			echo form_input_ledger('ledger_id[' . $i . ']', 0, 'tabindex="'.$tabindex.'"', $type = 'bankcash');
		else if ($add_type == 'nobankcash')
			echo form_input_ledger('ledger_id[' . $i . ']', 0, 'tabindex="'.$tabindex.'"', $type = 'nobankcash');
		else
			echo form_input_ledger('ledger_id[' . $i . ']','','tabindex="'.$tabindex.'"');
		echo '</td>';
		$tabindex++;

		echo '<td>';
		$dr_amount['tabindex'] = $tabindex;
		echo form_input($dr_amount);
		$tabindex++;
		echo '</td>';
		echo '<td>';
		$cr_amount['tabindex'] = $tabindex;
		...
		echo '</td>';
		echo '<td><a class="btn_sub_ledger" style="display:none;">Sub Ledger</a><span class="sub_ledger_form_data"></span></td>';
		echo '</tr>';
		return;
	}
function print_bill(){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($this->input->post('reverse_entry')==1){
				$_POST['ledger_id'][1] = $this->input->post('cash_ledger_id');
				$_POST['ledger_id'][0] = $temp['ledger_id'][0];
			}else{
...
function _check_account($entry_type){
		...
		if($entry_type=='journal' || $entry_type=='receipt' || $entry_type=='sales' || $entry_type=='salesreturn' || $entry_type=='purchase'){
			/* Entry Type */
			$entry_type_id = entry_type_name_to_id($entry_type);
			$current_entry_type = entry_type_info($entry_type_id);
			...
			foreach ($data_all_ledger_dc as $id => $ledger_data)
			{
				...
				if ($valid_ledger_q->num_rows() < 1)
				{
					...
					if($this->input->post('sales_type')==2){
						/* Check for valid ledger type */
						$valid_ledger = $valid_ledger_q->row();
						if ($current_entry_type['bank_cash_ledger_restriction'] == '2')
						{
	...
	function create_bill(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			/** For party*/
			$_POST['ledger_id'][0] = $temp['ledger_id'];
			$_POST['dr_amount'][0] = $temp['dr_amount'];
			$_POST['ledger_dc'][0] = $temp['ledger_dc'];
			...
			$_POST['sales_type'] = $temp['sales_type'];
			
			/** Check for journal entry*/
			$data_journal  = $this->_check_account('journal');	
			if($data_journal['error']==false){
				$ldgr_id = $temp['ledger_id'];
				$ldgr_dc = $temp['ledger_dc'];
				$ldgr_amt = $temp['dr_amount'];
			...
			if($data_journal['error'] == false){
				$sql = "CALL sp_account_create_bills($ldgr_id ,$ldgr_amt ,$sales_ldgr_id,$sales_cr_amt ,$vat_ldgr_id ,$vat_cr_amt ,".$this->session->userdata('sc_id').",".$this->session->userdata('user_id').",$sales_id,'$ldgr_dc','$sales_dc','$vat_dc')";		
				$result = $this->db->query($sql);
				$result->free_result();	
				
				$this->session->set_flashdata('success_save','Bill generated successfully');	
			}else{
				$this->session->set_flashdata('success_save',$data_journal['message']);
			}
			redirect('sales/sale/'.$sales_id);
		}
	}
	
function create_bill_check(){
		$this->load->library('form_validation');
		$this->form_validation->set_rules('sales_id', 'Sales ID', 'required');
		if ($this->form_validation->run() == FALSE){
			redirect('sales');
			...
			/** Check for journal entry*/
			$data_journal['data']  = $this->_check_account('sales');	
			$data_journal['temp_values'] = $temp;
			return $data_journal;
		}
	}
	
	function create_purchase_account(){
		$this->load->library('form_validation');
		...
		if ($this->form_validation->run() == FALSE){
			...
			/** For purchase*/
			$_POST['ledger_id'][1] = $temp['purchase_ledger_id'];
			$_POST['dr_amount'][1] = $temp['purchase_amount'];
			$_POST['ledger_dc'][1] = $temp['purchase_ledger_dc'];
			...
			$entry_type = 'journal';
			$data_journal = $this->_check_account($entry_type);
			...
			if($data_journal['error']==false){
				...
				$purchase_ldgr_id = $temp['purchase_ledger_id'];
				$purchase_cr_amt = $temp['purchase_amount'];
				$purchase_dc = $temp['purchase_ledger_dc'];
			...
			$_POST = $temp;
			if($data_journal['error'] == false){
				$sql = "CALL sp_accout_purchase($ldgr_id,".$ldgr_amt.",'".$ldgr_dc."',$purchase_ldgr_id,'".$purchase_dc."','".$purchase_cr_amt."','".$vat_ldgr_id."','".$vat_cr_amt."','".$vat_dc."',".$this->session->userdata('user_id').",$purchase_id)";		
				$result = $this->db->query($sql);
				$result->free_result();	
				return  $data_journal;				
			}else{
...
function sales_return(){
		...
		/** For sales*/
		$_POST['ledger_id'][1] = $temp['sales_return_ledger_id'];
		$_POST['dr_amount'][1] = $temp['sales_return_amount'];
		$_POST['ledger_dc'][1] = $temp['sales_return_ledger_dc'];
		...
		if($data_journal['error']==false){
			...
			$sales_ldgr_id = $temp['sales_return_ledger_id'];
			$sales_cr_amt = $temp['sales_return_amount'];
			$sales_dc = $temp['sales_return_ledger_dc'];
		...
		if($data_journal['error'] == false){
			$sql = "CALL sp_account_sales_return($ldgr_id ,$ldgr_amt ,$sales_ldgr_id,$sales_cr_amt ,$vat_ldgr_id ,$vat_cr_amt ,".$this->session->userdata('sc_id').",".$this->session->userdata('user_id').",$sales_return_id,'$ldgr_dc','$sales_dc','$vat_dc')";
			
			$result = $this->db->query($sql);
			$result->free_result();	
			$this->session->set_flashdata('success_save','sales returned successfully');	
			return $data_journal;
