...
class Report extends Account_Controller {
	var $acc_array;
	var $account_counter;
	...
	function index()
	{
		...
		$this->load->view('account/report/index');
		return;
	}
	...
	function balancesheet($period = NULL)
	{
		...
		$data['nav_links'] = array('account/report/download/balancesheet' => 'Download CSV', 'account/report/printpreview/balancesheet' => 'Print Preview');
		$data['left_width'] = "450";
		$data['right_width'] = "450";
		$this->load->view('account/report/balancesheet', $data);
		return;
	}
	...
	function trialbalance($period = NULL)
	{
		...
		$data['nav_links'] = array('account/report/download/trialbalance' => 'Download CSV', 'account/report/printpreview/trialbalance' => 'Print Preview');
	...
	function ledgerst($ledger_id = 0)
	{
		...
		if ($ledger_id != 0)
			$data['nav_links'] = array('report/download/ledgerst/' . $ledger_id  => 'Download CSV', 'report/printpreview/ledgerst/' . $ledger_id => 'Print Preview');

		if ($_POST)
		{
			$ledger_id = $this->input->post('ledger_id', TRUE);
			redirect('account/report/ledgerst/' . $ledger_id);
		}
		...
		if ($data['ledger_id'] > 0)
		{
			...
			if ($this->db->get()->num_rows() < 1)
			{
				...
				redirect('account/report/ledgerst');
				return;
			}
		...
		$this->load->view('account/report/ledgerst', $data);
		return;
	}
	...
	function reconciliation($reconciliation_type = '', $ledger_id = 0)
	{
		...
		if ($reconciliation_type == 'all')
		{
			...
			$data['show_all'] = TRUE;
			if ($ledger_id > 0)
				$data['nav_links'] = array('account/report/download/reconciliation/' . $ledger_id . '/all'  => 'Download CSV', 'report/printpreview/reconciliation/' . $ledger_id . '/all' => 'Print Preview');
		} else if ($reconciliation_type == 'pending') {
			$data['reconciliation_type'] = 'pending';
			$data['show_all'] = FALSE;
			if ($ledger_id > 0)
				$data['nav_links'] = array('account/report/download/reconciliation/' . $ledger_id . '/pending'  => 'Download CSV', 'report/printpreview/reconciliation/' . $ledger_id . '/pending'  => 'Print Preview');
		} else {
			$this->session->set_flashdata('custom_error','Invalid path.');
			redirect('account/report/reconciliation/pending');
			return;
		}
		...
		if ($data['ledger_id'] > 0)
		{
			...
			if ($this->db->get()->num_rows() < 1)
			{
				$this->session->set_flashdata('custom_error', 'Invalid Ledger account or Reconciliation is not enabled for the Ledger account.');
				redirect('account/report/reconciliation/' . $reconciliation_type);
				return;
			}
		} else if ($data['ledger_id'] < 0) {
		...
		if ($_POST)
		{
			...
			if ($_POST['submit'] == 'Submit')
			{
				...
				{
					redirect('account/report/reconciliation/all/' . $ledger_id);
					return;
				} else {
					redirect('account/report/reconciliation/pending/' . $ledger_id);
					return;
				}
				...
				if ($this->form_validation->run() == FALSE)
				{
					...
					$this->load->view('report/reconciliation', $data);
					return;
				} else {
					...
					foreach ($data_reconciliation_date as $id => $row)
					{
						...
						if ( ! $this->db->where('id', $id)->update('entry_items', $update_data))
						{
							...
							$this->form_validation->_error_array[] = 'Error updating reconciliation.';
							$this->logger->write_message("error", "Error updating reconciliation for entry item [id:" . $id . "]");
						} else {
							$this->db->trans_complete();
						}
					}
					$this->session->set_flashdata('success_save','Updated reconciliation.');
	...
	function printpreview($statement, $id = NULL)
	{
		...
		if ($statement == "reconciliation")
		{
			...
			if ($this->uri->segment(5) == 'all')
			{
				...
				$this->session->set_flashdata('custom_warning','Invalid path.');
			...
			if ($data['ledger_id'] > 0)
			{
				...
				if ($this->db->get()->num_rows() < 1)
				{
					$this->session->set_flashdata('custom_warning','Invalid Ledger account or Reconciliation is not enabled for the Ledger account.');
