...
if ( ! function_exists('date_php_to_mysql'))
{
	...
	{
		$CI =& get_instance();
		$current_date_format = $CI->mdl_mcb_data->setting('account_date_format');
		...
		case 'yyyy/mm/dd':
			...
			break;
		default:
			$CI->session->set_flashdata('custom_error','Invalid date format. Check your account settings.');
			return "";
		}
...
if ( ! function_exists('date_mysql_to_php'))
{
	function date_mysql_to_php($dt)
	{
		...
		case 'yyyy/mm/dd':
			...
			break;
		default:
			$CI->session->set_flashdata('warning','Invalid date format. Check your account settings.');
			return "";
		}
		return;
	}
...
if ( ! function_exists('date_today_php'))
{
	function date_today_php()
	{
		...
		$fy_end = $CI->mdl_mcb_data->setting('account_fy_end');
		if ($CI->mdl_mcb_data->setting('account_fy_start') > $todays_date)
			return date_mysql_to_php($fy_start);
		if ($CI->mdl_mcb_data->setting('account_fy_end') < $todays_date)
