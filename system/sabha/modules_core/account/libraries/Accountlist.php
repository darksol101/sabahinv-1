...
class Accountlist
{
	...
	{
		$CI =& get_instance();
		$CI->db->from('groups')->where('parent_id', $this->id)->where('(sc_id=0 OR sc_id='.$CI->session->userdata('sc_id').')');
	...
	function add_sub_ledgers()
	{
		...
		$CI->db->from('ledgers')->where('group_id', $this->id)->where('sc_id', $CI->session->userdata('sc_id'));
	...
	{
		$this->counter = $c;
		if($this->counter==0) { self::$cnt = 0;}		
		if ($this->id != 0)
		{
			$trstyle=self::$cnt%2==0?" odd": " even";
			self::$cnt++;
			echo "<tr class=\"tr-group$trstyle\">";
		...
		if (count($this->children_ledgers) > 0)
		{
			...
			foreach ($this->children_ledgers as $id => $data)
			{
				$trstyle=self::$cnt%2==0?" odd": " even";
				self::$cnt++;
				echo "<tr class=\"tr-ledger$trstyle\">";
				echo "<td class=\"td-ledger\">";
				echo $this->print_space($this->counter);
				echo "&nbsp;" . anchor('account/report/ledgerst/' . $data['id'], $data['name'], array('title' => $data['name'] . ' Ledger Statement', 'style' => 'color:#000000'));
				echo "</td>";
	...
	function account_st_main($c = 0)
	{
		...
		if ($this->id != 0)
		{
			...
			if ($this->id <= 4)
			{
				...
				echo "<td class=\"td-actions\">" . anchor('account/group/edit/' . $this->id , img(array('src' => asset_url() . "style/img/icons/edit.png", 'border' => '0', 'alt' => 'Delete group')), array('title' => 'Edit Group', 'class' => 'red-link'));
				echo " &nbsp;" . anchor('account/group/delete/' . $this->id, img(array('src' => asset_url() . "style/img/icons/delete_1.png", 'border' => '0', 'alt' => 'Delete group')), array('class' => "confirmClick", 'title' => "Delete Group")) . "</td>";
			}
			echo "</tr>";
		}
		foreach ($this->children_groups as $id => $data)
		...
		if (count($this->children_ledgers) > 0)
		{
			...
			foreach ($this->children_ledgers as $id => $data)
			{
				...
				echo "<td class=\"td-actions\">" . anchor('account/ledger/edit/' . $data['id'], img(array('src' => asset_url() . "style/img/icons/edit.png", 'border' => '0', 'alt' => 'Delete group')), array('title' => "Edit Ledger", 'class' => 'red-link'));
				echo " &nbsp;" . anchor('account/ledger/delete/' . $data['id'], img(array('src' => asset_url() . "style/img/icons/delete_1.png", 'border' => '0', 'alt' => 'Delete Ledger')), array('class' => "confirmClick", 'title' => "Delete Ledger")) . "</td>";
				echo "</tr>";
			}
			$this->counter--;
		}
