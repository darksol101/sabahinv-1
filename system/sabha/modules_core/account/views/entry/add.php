...
<h3 style="cursor: s-resize;"><?php
	echo 'New ' . $current_entry_type['name'] . ' Entry';
?></h3>
...
$(document).ready(function() {
	...
	$('table td .addrow').live('click', function() {
		...
		$.ajax({
			url: <?php echo '\'' . site_url('account/entry/addrow/' . $add_type) . '\''; ?>+'?unq='+ajaxunq(),
			success: function(data) {
...
<?php
	echo form_open('account/entry/add/' . $current_entry_type['label']);
	echo "<table width=\"60%\">";
	...
	echo "</span>";
	echo "<span id=\"tooltip-content-2\">Date format is " .  $this->mdl_mcb_data->setting('account_date_format') . ".</span>";
	echo '</td>';
	echo '</tr>';
	echo "</table>";
	...
	echo "<tr id=\"entry-total\"><td colspan=2><strong>Total</strong></td><td id=\"dr-total\">0</td><td id=\"cr-total\">0</td><td>" . img(array('src' => asset_url() . "style/img/icons/gear.png", 'border' => '0', 'alt' => 'Recalculate Total', 'class' => 'recalculate', 'title' => 'Recalculate Total')) . "</td><td></td><td></td></tr>";
