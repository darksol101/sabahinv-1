...
class Badparts extends Admin_Controller{
	...
	function badpartdetails(){
			$this->redir->set_last_index();
			$this->load->model(array('badparts/mdl_badpart_details','company/mdl_company'));
			$sc_id = $this->uri->segment('3');
			$part_number = $this->uri->segment('4');
			
			$this->mdl_badpart_details->getReturnPartDetails($sc_id,$part_number);
			
			
			
			}
	function getengineerbysc_defective_populate(){
		$sc_id = $this->input->post('sc_id');
		...
		$engineer_select  =  $this->mdl_html->genericlist($engineerOption,'engineer_id',array('class'=>'select-one','onchange'=>'getPartsByEngineer($(this).val());'),'value','text',$engineer_id);
		echo $engineer_select;
		}
		
	function getPartsByEngineer_defective_populate(){
            $this->redir->set_last_index();
			$engineer_id = $this->input->post('engineer_id');
			$part_number=$this->input->post('part_number');
            ...
            $part_select  =  $this->mdl_html->genericlist($partsOptions,'part_number',array('class'=>'validate[required]'),'value','text',$part_number);
            echo $part_select;
	}
		...
		$servicecenters_search=$this->mdl_html->genericlist( $scentersOptions, "sc_id_search",array('onchange'=>'getengineer($(this).val()); ','class'=>'validate[required] text-input'),'value','text',$sc_id_get);
			...
			$servicecenters_search=$this->mdl_html->genericlist( $scentersOptions, "sc_id_search",array('onchange'=>'getengineer($(this).val()); ','class'=>'validate[required] text-input'),'value','text',$this->session->userdata('sc_id'));
			}		
...
function getretrunlist()
		...
		$this->redir->set_last_index();
		$this->load->model(array('badparts/mdl_badparts_engineer_details','parts/mdl_parts','engineers/mdl_engineers','servicecenters/mdl_servicecenters'));
		$ajaxaction=$this->input->post('ajaxaction');
		$this->load->library('ajaxpagination');
		$currentpage = $this->input->post('currentpage');
		$config['base_url'] = base_url();
		...
		$page['limit'] = $config['per_page'];
		$page['start'] = $this->input->post('currentpage');
		$lists =  $this->mdl_badparts_engineer_details->getretrunlist($page);
...
function unsignedlist()
	{
		...
		$servicecenters_search=$this->mdl_html->genericlist( $scentersOptions, "sc_id_search",array('onchange'=>'getengineer($(this).val()); ','class'=>'validate[required] text-input'),'value','text',$sc_id);
...
{
		$this->redir->set_last_index();
		$this->load->model(array('company/mdl_company','engineers/mdl_engineers','badparts/mdl_badparts_engineer_details','parts/mdl_parts','servicecenters/mdl_servicecenters'));
		$ajaxaction=$this->input->post('ajaxaction');
		$this->load->library('ajaxpagination');
		$currentpage = $this->input->post('currentpage');
		$config['base_url'] = base_url();
		...
		$page['limit'] = $config['per_page'];
		$page['start'] = $this->input->post('currentpage');
		$lists =  $this->mdl_badparts_engineer_details->getUnsignedList($page);
   		$config['total_rows'] = $lists['total'];
		$this->ajaxpagination->cur_page=$this->input->post('currentpage');
		$this->ajaxpagination->initialize($config);
		$navigation = $this->ajaxpagination->create_links();
...
function create_excel_bad_unsign()
{
		$this->redir->set_last_index();
		ini_set("memory_limit","256M");
		$this->load->model(array('engineers/mdl_engineers','servicecenters/mdl_servicecenters','badparts/mdl_badparts_engineer_details','parts/mdl_parts'));
		$this->load->plugin('to_excel');
		$data = $this->mdl_badparts_engineer_details->getUnsignedDownload();
		$datas = array();
		$i=1;
		foreach ($data as $row){
			$row->sn= $i;
	...
	function excel_ready_unsigned_badparts(){
		...
		$this->load->model(array('engineers/mdl_engineers','servicecenters/mdl_servicecenters','badparts/mdl_badparts_engineer_details','parts/mdl_parts'));
		$this->load->plugin('to_excel');
		$data = $this->mdl_badparts_engineer_details->getReturnReportExcel();
		$datas = array();
		$i=1;
		foreach ($data as $row){
	...
	foreach ($result as $res){
		...
		$data['signed']=1;
		$this->mdl_badparts_engineer_details->save($data,$res);
		}
	}
	...
	function printReturnList() 
	{
		$this->redir->set_last_index();
	    $this->load->model(array('engineers/mdl_engineers','servicecenters/mdl_servicecenters','badparts/mdl_badparts_engineer_details','parts/mdl_parts'));
		$list =  $this->mdl_badparts_engineer_details->getRetunlist_print();
