...
function detailorder($badpart_orderid){
	...
	function saveOrder(){
		...
		$order_number = $this->getOrderNumber($this->input->post('badparts_from_sc_id'));
		$data['badparts_order_created_by']= $this->session->userdata('user_id');
		$data['badparts_order_number'] = $order_number->order_number;
		$data['badparts_order_created_ts'] = date('Y-m-d H:i:s');
		$data['badparts_order_status'] = 1;
		$data['badparts_order_dt'] = date('Y-m-d');
		parent::save($data);
	...
	function getOrderNumber($requestion_sc_id){
		$sql = "SELECT  CONCAT(UCASE(sc.sc_name),DATE_FORMAT(CURDATE(),'%d%m%Y'),
	LPAD(
		(SELECT COUNT(*)+1
FROM sst_badparts_orders AS bo WHERE badparts_order_created_ts <= NOW() AND badparts_order_created_ts > SUBDATE(NOW(),1) ),
	3,'0'),'BO') AS order_number
