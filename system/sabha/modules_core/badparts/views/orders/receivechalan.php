<?php //echo '<pre>'; print_r($transit_details); print_r($orderparts);?>
...
<td>  <input type="input" id="courior_number" readonly="readonly" name="courior_number" class="text-input" value="<?php echo $transit_details->badparts_chalan_number;?>" /></td>
...
//die();
	...
	$chalan_dispatched_quantity = $this->mdl_badparts_order_parts_details->getDispatchedPartByChalan($orderpart->badparts_order_part_id,$transit_details->badparts_transit_detail_id);
	//$order_parts_details = $this->mdl_order_parts_details->getPartOderDetailsByOrderParts($orderpart->order_part_id);
	$chalan_received_quantity = $this->mdl_badparts_order_parts_details->getReceivedQuantityByChalan($orderpart->badparts_order_part_id,$transit_details->badparts_transit_detail_id);
	$order_part_detail_id = $this->mdl_badparts_order_parts_details->orderPartDetailId($orderpart->badparts_order_part_id,$transit_details->badparts_transit_detail_id);
	$chalandifferance = $this->mdl_badparts_order_parts_details->getDifferance($orderpart->badparts_order_part_id,$transit_details->badparts_transit_detail_id);
	//echo '<pre>'; print_r($chalandifferance);
	//print_r($chalan_received_quantity);
?> 
<tr>
<td> <?php echo $i;?></td>
<td  style="text-align:center"><input type="hidden" id="badparts_order_part_number" value="<?php echo $orderpart->badparts_order_part_id;?>" /><?php echo $orderpart->part_number;?></td>
...
<td style="text-align:center"><input type="hidden" value="<?php echo $order_part_detail_id->badparts_order_part_details_id;?>"/><?php echo $chalan_dispatched_quantity->part_quantity; ?> </td>
...
<input type="hidden" id="hdn_requested_sc_id" value="<?php echo $transit_details->badparts_to_sc_id;?>" />
<input type="hidden" id="hdn_requesting_sc_id" value="<?php echo $transit_details->badparts_from_sc_id?>"/>
<input type="hidden" id="hdn_order_id" value=" <?php echo $transit_details->badparts_order_id;?>" />
</form>
</div>
...
<table align="right" style="margin-top:11px">
	<tr>
		<td style="text-align: right; font-size: 11px;"><input type="button"
			...
			onclick="partialordercard(<?php echo $transit_details->badparts_transit_detail_id?> ,<?php echo $transit_details->badparts_order_id?>);" /></td>
		<td style="text-align: right; font-size: 11px;"><input type="button"
