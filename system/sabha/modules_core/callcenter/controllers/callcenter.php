...
function create_excel()
	{
		...
		$this->load->model(array('parts/mdl_parts','parts/mdl_parts_used','productmodel/mdl_productmodel','brands/mdl_brands','cities/mdl_cities','engineers/mdl_engineers','servicecenters/mdl_servicecenters','products/mdl_products','mcb_data/mdl_mcb_data','customers/mdl_customers','faultsettings/mdl_defect','faultsettings/mdl_repair','faultsettings/mdl_Symptom','productmodel/mdl_product_serial_number'));
		$this->load->model('users/mdl_users');
		$this->load->plugin('to_excel');
		$this->load->helper('calls');
		$pg['start'] = 0;
		$list = $this->mdl_callcenter->getCallsDownload($pg);
		$result = $list['result'];
		$data = '';
		...
		foreach($result->result() as $row){
			...
			if($row->call_status == 'closed') {
				$average_closing =  CalculateAvgClosingTimeStamp($row->call_dt,$row->call_tm,$row->closure_dt,$row->closure_tm);
					$row->call_aging=	 CalculateSecondsToDays($average_closing);
					}
				else {
		...
		$fields = array('Call ID','Reg Date/Time','First Name','Last Name','City','Address','Office Phone','Home Phone','Mobile Number','Brand','Product','Model Number','Engineer','Aging Date/Time','Store',
						...
						'Complain Remark','Engineer Remark','Details of work done','Used Item Number','Item description','Quantity','Pending Reason','Status','Pending Date/Time','Closure Date/Time','Registered By','Call Last Modified By','Serial Number','Old Serial Number','Purchase Date','Call Type','Dealer Name','Call At','Call From','Service Type','Reopened','Happy Call');
	...
	function _post_handler(){
		if($this->input->post('btn_save')){
			...
			if((int)$call_id==0){
				...
				$district = $this->mdl_callcenter->getDistrict_id($sc_id);
				...
				if($this->mdl_callcenter->save_call($data_call,0,$district)){
				...
				if(is_array($used_parts_numbers)){
					foreach($used_parts_numbers as $used_part_number){
						if($used_part_number && $used_parts_quantity >=1){
							$company_id = $this->mdl_company->getcompanyid(trim($used_parts_company[$j]));
							//$company_id =5;
				...
				//die();
								if($check_parts_stock->stock_quantity< $used_parts_quantity[$j]){									
									...
									$this->mdl_stocks->updateBadparts($stockdata);
								}
							}
						}
						$j++;
	...
	function savecalldatess(){
		...
		$this->redir->set_last_index();
         $this->load->model(array('engineers/mdl_engineers','servicecenters/mdl_servicecenters','mcb_data/mdl_mcb_data', 'utilities/mdl_html','products/mdl_products','productmodel/mdl_productmodel','customers/mdl_customers'));
	    $ajaxaction=$this->input->post('ajaxaction');
		$this->load->library('ajaxpagination');
		$currentpage = $this->input->post('currentpage');
		$config['base_url'] = base_url();
		...
		$page['limit'] = $config['per_page'];
		$page['start'] = $this->input->post('currentpage');
		 $result = $this->mdl_callcenter->getreport211($page);
		 $lists = $result['list'];
        ...
        $this->load->view('reports/report211/report211list',$data);        
		
		
		}
		
		function dispatchDevice(){
	$this->redir->set_last_index();
	$call_id = $this->input->post('call_id');
	$data['dispatch'] ='1';
	$this->mdl_callcenter->save($data,$call_id);	
