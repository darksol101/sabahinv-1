...
class Callcenter extends Admin_Controller {
	...
	function create_excel()
	{
		...
		$this->load->model(array('productmodel/mdl_productmodel','brands/mdl_brands','cities/mdl_cities','settings/mdl_cancellation','engineers/mdl_engineers','servicecenters/mdl_servicecenters','products/mdl_products','mcb_data/mdl_mcb_data','customers/mdl_customers','faultsettings/mdl_defect','faultsettings/mdl_repair','faultsettings/mdl_Symptom'));
...
if ($row->call_status == 'cancelled' && is_numeric($row->call_reason_pending)){ 
				 $reason = $this->mdl_cancellation->getcancellationreason($row->call_reason_pending);
				 $row->call_reason_pending = $reason->cancellation;
				}}
		...
		$fields = array('Call ID','Reg Date/Time','First Name','Last Name','City','Address','Office Phone','Home Phone','Mobile Number','Brand','Product','Model Number','Engineer','Aging Date/Time','Store',
						'Symptom Code','Defect Code','Repair Code',
	...
	function callregistration(){
	   ...
	   $call_reason_pending_select  =  $this->mdl_html->genericlist($pendingOptions,'call_reason_pending',array('onchange'=>'showpartbtn();'),'value','text',$call_details->call_reason_pending);
	   /**
	...
	function getreasonlist()
	{
		...
		if($call_status==1){
			...
			$pendingOptions = $this->mdl_reasons->getPendingOptions();
			array_unshift($pendingOptions, $this->mdl_html->option( '', 'Select Reasong For Pending'));
			$call_reason_pending_select  =  $this->mdl_html->genericlist($pendingOptions,'call_reason_pending',array('onchange'=>'showpartbtn();','class'=>'validate[required] select-one'),'value','text',$active);
		...
		if($this->input->post('btn_save')){
			...
			//print_r($defected_company);
				...
				if($this->mdl_callcenter->save($data_call,$call_id)){
					$this->session->set_flashdata('save_status',true);
					$this->session->set_flashdata('call_save',$this->lang->line('this_item_has_been_saved'));
				}else{
					$this->session->set_flashdata('save_status',false);
					$this->session->set_flashdata('call_save',$this->lang->line('this_item_not_saved'));
				}
