...
class Mdl_Call_happy extends MY_Model {
	...
	$var=$this->db->query('SELECT DISTINCT pm.model_id FROM sst_prod_models AS pm JOIN sst_calls AS hcd ON hcd.`model_id`=pm.model_id AND pm.product_id IN('.$product_id.')');
		...
		if($todate)
				{			
			$this->db->where('hcd.call_last_mod_ts <=', date("Y-m-d 23:59:59",date_to_timestamp($todate)));	
				}	
				
		if(isset($page['limit']))		{
			$this->db->limit((int)$page['limit'],(int)$page['start']);
										}
			$this->db->select('hcd.call_id,hcd.call_uid');		
			$this->db->from($this->mdl_callcenter->table_name.' AS hcd');
	...
	$var=$this->db->query('SELECT DISTINCT pm.model_id FROM sst_prod_models AS pm JOIN sst_calls AS hcd ON hcd.`model_id`=pm.model_id AND pm.product_id IN('.$product_id.')');
		...
		if($todate)
			...
			$this->db->select('hcd.call_id,hcd.call_uid,hcd.call_last_mod_by AS modified_by,hcd.call_last_mod_ts AS modified_date');		
			$this->db->from($this->mdl_callcenter->table_name.' AS hcd');
			$this->db->where('hcd.happy_status',1);
