...
class Mdl_Callcenter extends MY_Model {
	...
	public function getCalls($page){
		...
		//	$this->db->like("concat(c.call_serial_no,csn.call_serial_no)",$sn);
			//$this->db->or_like('csn.call_serial_no',$sn);
		}
		if($ph){
		...
		if($this->session->userdata('usergroup_id')>1 && $this->session->userdata('usergroup_id') !=6 && $this->session->userdata('usergroup_id') !=8 && $this->session->userdata('usergroup_id') !=3){
...
$this->db->select('c.call_id,c.call_uid,c.call_dt,c.call_tm,cs.cust_first_name,cs.cust_last_name,ct.city_name,cs.cust_address,cs.cust_phone_office,cs.cust_phone_home,cs.cust_phone_mobile,b.brand_name,p.product_name,pm.model_number,en.engineer_name,c.call_dt AS call_aging,sc_name,sym.symptom_code,def.defect_code,rep.repair_code,c.call_service_desc,c.call_engineer_remark,c.call_detail_wrk_done,c.call_reason_pending,c.call_status,c.pending_dt,c.pending_tm,c.closure_dt,c.closure_tm,c.call_created_by,c.call_last_mod_by,c.call_serial_no,csn.call_serial_no as call_old_serial_no,call_purchase_dt,call_type,call_dealer_name,c.call_last_mod_ts,c.call_at,c.call_from,c.call_service_type,reopened,c.happy_status');
		...
		$this->db->join($this->mdl_product_serial_number->table_name.' AS csn','csn.call_id = c.call_id','left');
		$this->db->order_by('call_status asc,c.call_id desc');
		if(isset($page['limit'])){
...
function generateCallId(){
		...
		$num = " select (LPAD((SELECT COUNT(*)+1
FROM sst_calls AS c WHERE c.call_created_ts <= NOW() AND c.call_created_ts > SUBDATE(NOW(),1) ),3,'0')) as num";
			$number =$this->db->query($num);
			$id = $this->session->userdata('zone_code').date('d').date('m').date('y').$number->row()->num ;
...
FROM sst_calls AS c WHERE c.call_created_ts <= NOW() AND c.call_created_ts > SUBDATE(NOW(),1) ),3,'0')) as num";
			$number =$this->db->query($num);
			$id = $zone_code.date('d').date('m').date('y').$number->row()->num ;
			return $id;
		}else{
			return false;
		}
...
public function getCallsDownload($page){
		...
		if($this->session->userdata('usergroup_id')>1 && $this->session->userdata('usergroup_id') !=6 && $this->session->userdata('usergroup_id') !=7){
			$this->db->where("p.product_id IN (".$this->session->userdata('user_products').")");
		}
		if($pn){
			$this->db->like('p.product_name',$pn);
...
$this->db->select('c.call_id,c.call_uid,c.call_dt,c.call_tm,cs.cust_first_name,cs.cust_last_name,ct.city_name,cs.cust_address,cs.cust_phone_office,cs.cust_phone_home,cs.cust_phone_mobile,b.brand_name,p.product_name,pm.model_number,en.engineer_name,c.call_dt AS call_aging,sc_name,sym.symptom_code,def.defect_code,rep.repair_code,c.call_service_desc,c.call_engineer_remark,c.call_detail_wrk_done,pus.part_number,pt.part_desc,pus.part_quantity,c.call_reason_pending,c.call_status,c.pending_dt,c.pending_tm,c.closure_dt,c.closure_tm,c.call_created_by,c.call_last_mod_by,call_serial_no,call_serial_no as call_old_serial_no,call_purchase_dt,call_type,call_dealer_name,c.call_last_mod_ts,c.call_at,c.call_from,c.call_service_type,reopened,c.happy_status');
		$this->db->from($this->table_name.' as c',$this->mdl_productmodel->table_name.' as pm',$this->mdl_brands->table_name.' as b',$this->mdl_products->table_name.' as p',$this->mdl_customers->table_name.' as cs');
		$this->db->join($this->mdl_productmodel->table_name.' as pm','pm.model_id=c.model_id','left');
		$this->db->join($this->mdl_brands->table_name.' as b','b.brand_id=c.brand_id','left');
		$this->db->join($this->mdl_products->table_name.' as p','p.product_id=pm.product_id','left');
		...
		$this->db->join($this->mdl_symptom->table_name.' as sym','sym.symptom_id = c.symptom_id','left');
		$this->db->join($this->mdl_repair->table_name.' as rep','rep.repair_id = c.repair_id','left');
		$this->db->join($this->mdl_parts_used->table_name.' AS pus','pus.call_id = c.call_id','left');
		$this->db->join($this->mdl_parts->table_name.' AS pt','pt.part_number = pus.part_number','left');
...
$cid=$this->input->get('cid');
		...
		if($cn){
			//$this->db->like('cs.cust_first_name',$cn);
			//$this->db->like('cs.cust_last_name',$cn);
			$this->db->like("concat(cs.cust_first_name,cs.cust_last_name)",$cn);
		}
		if($pn){
			$this->db->like('p.product_name',$pn);
		...
		if($ph){
			//$this->db->like('cs.cust_phone_home',$ph);
			//$this->db->like('cs.cust_phone_office',$ph);
			//$this->db->like('cs.cust_phone_mobile',$ph);
			$this->db->like("concat(cs.cust_phone_mobile ,cs.cust_phone_office ,cs.cust_phone_home)",$ph);
		}
		if($eg!=''){
			$this->db->where('c.engineer_id =',$eg);
		...
		$this->db->where('b.brand_status =','1');
		/*if($call_status!=''){
			$this->db->where('c.call_status =',$call_status);
			}*/
	$arr = explode("_",$call_status);
		$call_status = implode(",",$arr);
    ...
    function getDistrict_id($sc_id){
		
		$this->load->model(array('servicecenters/mdl_servicecenters','cities/mdl_cities','zones/mdl_districts'));
		$this->db->select('sc.sc_name,ct.district_id');
		$this->db->from($this->mdl_servicecenters->table_name.' AS sc');
		$this->db->where('sc.sc_id =',$sc_id);
		$this->db->join($this->mdl_cities->table_name.' AS ct','ct.city_id = sc.city_id');
		$result = $this->db->get();
	//	echo $this->db->last_query();
		return $result->row()->district_id;
	}
