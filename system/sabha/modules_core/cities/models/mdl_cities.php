...
class mdl_Cities extends MY_Model {
	...
	public function getCitiesByDistrict($districts){
		...
		$this->db->select('city_id,city_name,district_name,d.district_id,(SELECT count(city_id) FROM sst_auto_ass_city AS CT WHERE CT.city_id=c.city_id AND CT.sc_id="'.$this->input->post('sc_id').'") as selected');
	...
	public function getCitiesByDistrictAssigns($districts,$sc_id){
		if(empty($districts)){
			$districts = '0';
		}
		$this->db->select('city_id,city_name,district_name,d.district_id,(SELECT count(city_id) FROM sst_auto_ass_city AS CT WHERE CT.city_id=c.city_id AND CT.sc_id="'.$sc_id.'") as selected,d.zone_id');
		$this->db->from($this->table_name.' AS c',$this->mdl_districts->table_name.' AS d');
		$this->db->join($this->mdl_districts->table_name.' AS d','d.district_id = c.district_id','left');
		$this->db->where("c.district_id IN ($districts)");
		$this->db->order_by('district_name ASC,c.city_name ASC');
