...
<div class="toolbar1">
  ...
  <table width="100%">
  	...
  	<tr>
    	<td>Search</td>
        <td><input type="text" name="searchtxt" id="searchtxt" class="text-input" value="" onKeydown="Javascript: if (event.keyCode==13) {$('#currentpage').val(0);showCityList();}" /></td>
        <td><span id="ct_zone_search"><?php echo $zone_search; ?></span></td>
        <td><span id="ct_district_search"><?php echo $district_search; ?></span></td>
        <td><img src="<?php echo base_url();?>assets/style/img/icons/search.gif" style="margin-bottom: -8px;" class="searchbtn"  onclick="javascript:$('#currentpage').val(0);showCityList();" /></td>
    </tr>
  </table>
