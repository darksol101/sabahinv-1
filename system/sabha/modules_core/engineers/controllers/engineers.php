...
class Engineers extends Admin_Controller {
	...
	function index(){
		...
		/*
		** Allow Store to be able to view it's engineers and add too
		*/
		
		if($this->session->userdata('usergroup_id')==4){
			$servicecenterOptions = $this->mdl_servicecenters->getServiceCentersOptionsById($this->session->userdata('sc_id'));
		}else{
	...
	function saveengineer(){
		...
		$access_level = $this->mdl_engineers->restrictUserByGroup(4);
		if($access_level==true){
			...
			$error = array('type'=>'warning','message'=>'Unable to save engineer');
		}
