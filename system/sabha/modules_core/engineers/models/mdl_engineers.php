...
class mdl_Engineers extends MY_Model {
	...
	public function getEngineerNameById($id){
		$params=array(
					 "select"=>"engineer_name",
					 "where"=>array("engineer_id"=>(int)$id),
					 "limit"=>1
...
public function getEngineerOptionsBySc($sc_id){
		$params=array(
					 "select"=>"engineer_id as value,engineer_name as text",
					 "where"=>array("engineer_status"=>"1","sc_id" => $sc_id),
					 "order_by"=>'text'
...
function getengineer_call($engineer_id){
			$this->db->select('engineer_name');
...
function getengineersreport($sc_id){
		if(empty($sc_id)){
			$sc_id = '0';
		}
		$this->db->select('engineer_id as value,engineer_name as text');
		$this->db->from($this->table_name.' AS e');
		
		$this->db->where("e.sc_id IN ($sc_id)");
		//$this->db->order_by('b.brand_name ASC,p.product_name ASC');
		$result = $this->db->get();
		return $result->result();
	}

function getEngineerlist_alloca($id)
	{
		$this->db->select('*');
		$this->db->from($this->table_name.' AS e');
		$this->db->where('e.engineer_id =',$id);
		$result=$this->db->get();
		return $result->row();
	...
	// function
	public function getEngineerByid_alloc($id){
		$params=array(
					 "select"=>"engineer_id as value,engineer_name as text",
					 "where"=>array("engineer_status"=>"1","engineer_id" => $id),
					 "order_by"=>'text'
					 );
		$options=$this->get($params);
					 return $options;
