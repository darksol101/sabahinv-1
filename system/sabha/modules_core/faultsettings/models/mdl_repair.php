...
class mdl_Repair extends MY_Model {
	...
	public function getRepairlist($page)
	{
		...
		//search for symptom code
		if(!empty($defect_id)){
			$this->db->where("$table_defect.defect_id =", $defect_id); 
		}
	...
	{
		$this->db->select('r.repair_id,r.repair_code,r.repair_description,r.repair_status,s.symptom_id,r.defect_id,s.product_id,p.brand_id');
