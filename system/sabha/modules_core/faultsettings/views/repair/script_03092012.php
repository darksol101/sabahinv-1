...
function showrepairlist(){
	...
	var symptom_id = $("#symptom_search").val();
	var defect_id = $("#defect_search").val();
	
	var params="ajaxaction=getrepairlist&searchtxt="+searchtxt+"&currentpage="+currentpage+"&sort_field="+sort_field+"&sort_type="+sort_type+"&brand_id="+brand_id+"&product_id="+product_id+"&symptom_id="+symptom_id+"&defect_id="+defect_id+"&unq="+ajaxunq();
	$.ajax({			
			type	:	"POST",
...
function getSymptomsByProductSearch(product_id,active){
	loading("symptom_box_search");
	var params = 'product_id='+product_id+'&active='+active+'&unq='+ajaxunq();
	$.ajax({			
			type	:	"POST",
			url		:	"<?php echo base_url();?>faultsettings/repair/getsymptomsbyproductsearch",
			data	:	params,
			success	:	function (data){
				$("#symptom_box_search").html(data);
				hideloading();
				}								
		});
}
function getDefectCodeBySymptomSearch(symptom_id){
	loading("defect_box_search");
	var params = 'symptom_id='+symptom_id+'&unq='+ajaxunq();
	$.ajax({			
			type	:	"POST",
			url		:	"<?php echo base_url();?>faultsettings/repair/getdefectsbysymptomsearch",
			data	:	params,
			success	:	function (data){
				$("#defect_box_search").html(data);
				hideloading();
				}								
		});
}
