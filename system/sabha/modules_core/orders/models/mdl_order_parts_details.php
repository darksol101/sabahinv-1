...
class Mdl_Order_parts_details extends MY_Model
{
	public function __construct()
	{
		parent::__construct();
		$this->table_name = 'sst_order_parts_details';
		$this->primary_key		=	'sst_order_parts_details.order_part_details_id';
		$this->select_fields	= 'SQL_CALC_FOUND_ROWS *';
		$this->order_by			= 'order_part_details_id';
	...
	function getDispatchedQuantity($order_part_id){
	...
	function getDeliveredQuantity($order_part_id){
	...
	function getDispatchedPartByChalan($order_part_id,$transit_id){
		...
		$this->db->from($this->table_name.' AS pd');
		$this->db->where('pd.order_part_id =',$order_part_id);
		$this->db->where('pd.transit_id =',$transit_id);
		//$this->db->where('pd.order_part_status = 0');
	...
	function orderPartDetailId($order_part_id,$transit_id){
		$this->db->select('pd.order_part_details_id');
		$this->db->from($this->table_name.' AS pd');
		$this->db->where('pd.order_part_id =',$order_part_id);
		$this->db->where('pd.transit_id =',$transit_id);
		$result = $this->db->get();
	...
	function getOrderPartDetailTransit($order_part_detail_id){
		$this->db->select('pd.part_quantity,pd.differance,pd.order_part_status,pd.differance');
		$this->db->from($this->table_name.' AS pd');
		$this->db->where('pd.order_part_details_id =',$order_part_detail_id);
		$result = $this->db->get();
		$data = $result->row();
		return $data;
