...
class Mdl_Orders extends MY_Model
{
	...
	$this->load->model('servicecenters/mdl_servicecenters');
		$requested_sc_id = $this->mdl_mcb_data->get('main_service_center');
			...
			$query="SELECT COUNT(order_id) AS count_order FROM sst_orders WHERE DATE_FORMAT(order_created_ts,'%Y-%m-%d') = DATE_FORMAT(CURDATE(), '%Y-%m-%d')";
			$count_order = $this->db->query($query);
			$order_count = $count_order->row();
			$final_count = $order_count->count_order + 1;
			if($final_count<=9){$final_count="0".$final_count; }
			$result_sc = $this->mdl_servicecenters->getscdetailsbyid($db_array[0]->sc_id);
		$data['order_number']=strtoupper($result_sc->sc_name).date('dmY').$final_count.'RO';
	...
	function saveOrder(){
		...
		$part_number_arr= $this->mdl_parts->getPartsId($part_number_arr);
	...
	if($order_id==0){
			...
			$result=$this->mdl_servicecenters->getscdetailsbyid($this->input->post('requesting_sc_id'));
			$query="SELECT COUNT(order_id) AS count_order FROM sst_orders WHERE DATE_FORMAT(order_created_ts,'%Y-%m-%d') = DATE_FORMAT(CURDATE(), '%Y-%m-%d')";
			...
			$final_count = $order_count->count_order + 1;
			if($final_count<=9){$final_count="0".$final_count; }
			$data['order_number']=strtoupper($result->sc_name).date('dmY').$final_count.'PO'; 
	...
	function thisorderpicking($id){
		
		$this->db->select('order_id,order_number,order_dt,call_id,requested_sc_id,order_status,sc.sc_name');
		$this->db->from($this->table_name);
		$this->db->join($this->mdl_servicecenters->table_name.' AS sc','sc.sc_id= requested_sc_id');
		$this->db->where('order_id =',$id);
		$result= $this->db->get();
		$result= $result->row();
		return $result;
	...
	function getOrderReport(){
		...
		$from_sc_id = $this->input->post('sc_id_from');
		$to_sc_id = $this->input->post('sc_id_to');
		$from_date = $this->input->post('from_date');
		$to_date = $this->input->post('to_date');
		
		if($from_sc_id){
			$this->db->where('o.requested_sc_id =',$from_sc_id);
		}
		if($to_sc_id){
			$this->db->where('o.requesting_sc_id =',$to_sc_id);
		}
		if($to_date){
			$this->db->where('order_parts_details_created_ts <=', date("Y-m-d",date_to_timestamp($to_date)));
		}
		if($from_date){
			$this->db->where('order_parts_details_created_ts >=', date("Y-m-d",date_to_timestamp($from_date)));
		}
		
		$this->db->select('op.part_id,p.part_number,sum(opd.part_quantity)as quantity,sum(opd.differance) as diff,sc.sc_name,p.part_desc,sum(opd.part_quantity)+sum(opd.differance) as dispatched_quantity, SUM(CASE WHEN(opd.order_part_status=1)THEN opd.part_quantity END) as delivered_quantity',false);
		$this->db->from($this->mdl_order_parts_details->table_name.' AS opd');
		$this->db->join($this->mdl_order_parts->table_name.' AS op','op.order_part_id = opd.order_part_id');
		$this->db->join($this->table_name.' AS o','o.order_id = op.order_id');
		$this->db->join($this->mdl_servicecenters->table_name.' AS sc','sc.sc_id = o.requesting_sc_id');
		$this->db->join($this->mdl_parts->table_name.' AS p','p.part_id = op.part_id');
		$this->db->group_by('op.part_id');
		$result = $this->db->get();
		//echo $this->db->last_query();
	...
	function getDetailOrderReport($from_date,$to_date,$from_sc_id,$to_sc_id,$part_number){
		...
		if($to_date){
			$this->db->where('order_parts_details_created_ts <=', $to_date);
		}
		if($from_date){
			$this->db->where('order_parts_details_created_ts >=', $from_date);
		}
		
		$this->db->select('opd.part_quantity,opd.order_part_status,opd.differance,p.part_number,o.order_number,sc_name,part_desc');
		$this->db->from($this->mdl_order_parts_details->table_name.' AS opd');
		$this->db->join($this->mdl_order_parts->table_name.' AS op','op.order_part_id = opd.order_part_id');
		$this->db->join($this->table_name.' AS o','o.order_id = op.order_id');
		$this->db->join($this->mdl_servicecenters->table_name.' AS sc','sc.sc_id = o.requesting_sc_id');
		$this->db->join($this->mdl_parts->table_name.' AS p','p.part_id = op.part_id');
		$this->db->where('op.part_id =', $part_number);
		$result = $this->db->get();
		//echo $this->db->last_query();
	    return 	$result->result();
