...
<?php $this->load->model(array('orders/mdl_order_parts_details'));?>
<script>
	$(function() {
...
<col width="1%" /><col width="15%"  /><col width="30%" /><col width="10%" /><col width="5%" /><col width="5%" /><col width="10%" /><col width="16%" />
<thead>
	<tr>
...
<tr>
<td> <?php echo $i;?></td>
<td  style="text-align:left"><input type="hidden" id="order_part_number" value="<?php echo $orderpart->order_part_id;?>" /><?php echo $orderpart->part_number;?></td>
...
<td  style="text-align:center"><input type="hidden" name="remaining_quantity" value="<?php echo ($orderpart->part_quantity - ($dispatched_quantity->dispatched_quantity)) ?>" /><?php echo $orderpart->part_quantity;?> </td>
...
<td style="text-align:center"><?php if ($this->session->userdata('sc_id') == $requested_sc_id) {?><a title="Confirm" class="btn confirm_part">Dispatch</a><?php }?></td>
</tr>
<?php  }?>
</tbody>
</table>
<input type="hidden" id="hdn_requested_sc_id" value="<?php echo $requested_sc_id;?>" />
<input type="hidden" id="hdn_requesting_sc_id" value="<?php echo $requesting_sc_id?>"/>
<input type="hidden" id="hdn_order_id" value=" <?php echo $order_id;?>" />
</form>
</div>
