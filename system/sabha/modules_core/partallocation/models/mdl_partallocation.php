...
class Mdl_Partallocation extends MY_Model {
	...
	function getallocatedlist ($page){
		...
		if ($searchtxt){
			//$this->db->like('ps.part_number',$searchtxt);
			$this->db->where("(a.part_number like '%".$searchtxt."%' or pd.part_desc like '%".$searchtxt."%' )");
		//$this->db->like('concat(a.part_number,pd.part_desc)',$searchtxt);
			}
		if ($sc_id){
		...
		$this->db->select ('e.engineer_name, a.part_number, c.company_title, a.allocated_quantity,a.engineer_id,a.company_id,a.sc_id,sc.sc_name,pd.part_desc');
		$this->db->from($this->table_name.' AS a');
		$this->db->join($this->mdl_company->table_name.' AS c','c.company_id = a.company_id','INNER');
		$this->db->join($this->mdl_parts->table_name.' AS pd','pd.part_number = a.part_number','INNER');
		...
		$this->db->where('a.allocated_quantity >',0);
...
function getAvailableStocks($engineer_id,$sc_id,$page){
	
		$upart_search=$this->input->post('upart_search');
		if($upart_search){
			$this->db->like('concat(ps.part_number," ",pp.part_desc)',$upart_search);
		}
	...
	{   
		...
		$check_unsign = $this->input->post('check_unsign');
		$st_check=$this->input->post('st_check');
		//$part_number=$this->input->post('part_number');
		$sc_id = $this->input->post('sc_id');
		$engineer = $this->input->post('engineer');
		...
		if(	$st_check == "Revoked")
		{
			$this->db->select ('pd.parts_allocation_details_id,e.engineer_name, a.part_number,pdc.part_desc, a.allocated_quantity,sc.sc_name, pd.created_date,pd.signed,pd.quantity_out as quantity');
			$this->db->where('pd.event =','Revoked');
			
		} else{	
				
		if($allco_select==1)
		{	$this->db->select ('pd.parts_allocation_details_id,e.engineer_name, a.part_number,pdc.part_desc, a.allocated_quantity,sc.sc_name, pd.created_date,pd.signed,pd.quantity_in as quantity');
		...
		$this->db->join($this->mdl_parts->table_name.' AS pdc','pd.part_number = pdc.part_number');
		...
		$check_unsign = $this->input->get('check_unsign');
		...
		$st_check=$this->input->get('st_check');
	   if($searchdate){
		$this->db->where('date(pd.created_date) =',$searchdate);
				}
	   if(	$st_check == "Revoked")
	...
	function getUnsignedList($page){
		...
		$this->db->select ('e.engineer_name, e.engineer_id,a.part_number, pdc.part_desc,a.allocated_quantity,sc.sc_name,pd.event, pd.created_date,pd.signed,pd.quantity_in,pd.quantity_out,sc.sc_id,');
		$this->db->from($this->mdl_partallocation_details->table_name.' AS pd');
		...
		$this->db->select ('e.engineer_name, a.part_number,pdc.part_desc, a.allocated_quantity,sc.sc_name,pd.event, pd.created_date,pd.signed,pd.quantity_in,pd.quantity_out');
		$this->db->from($this->mdl_partallocation_details->table_name.' AS pd');
		$this->db->join($this->mdl_engineers->table_name.' AS e','e.engineer_id = pd.engineer_id','INNER');
		$this->db->join($this->table_name.' AS a','a.part_number =pd.part_number AND a.engineer_id = pd.engineer_id');
		$this->db->join($this->mdl_servicecenters->table_name.' AS sc','sc.sc_id = a.sc_id');
...
function getAllocReportExcel()
{
		...
		$this->db->select ('pd.parts_allocation_details_id as sn, sc.sc_name,e.engineer_name,a.part_number,pdc.part_desc,pd.event, pd.quantity_in + pd.quantity_out as quantity, pd.created_date');
		$this->db->from($this->mdl_partallocation_details->table_name.' AS pd');
		$this->db->join($this->mdl_engineers->table_name.' AS e','e.engineer_id = pd.engineer_id','INNER');
		$this->db->join($this->table_name.' AS a','a.part_number =pd.part_number AND a.engineer_id = pd.engineer_id');
		$this->db->join($this->mdl_servicecenters->table_name.' AS sc','sc.sc_id = a.sc_id');
...
function getallocatedlist_excel (){
		$searchtxt = $this->session->userdata('ses_searchtext');
		$sc_id =$this->session->userdata('ses_sc_id');
		$company = $this->session->userdata('ses_company');
		$engineer = $this->session->userdata('ses_engineers');
		//print_r($this->input->post());
		if ($searchtxt){
			//$this->db->like('ps.part_number',$searchtxt);
			$this->db->where("a.part_number like '%".$searchtxt."%' or pd.part_desc like '%".$searchtxt."%' ");
		...
		$this->db->select ('e.engineer_name,sc.sc_name ,a.part_number,pd.part_desc, c.company_title, a.allocated_quantity');
		$this->db->from($this->table_name.' AS a');
		$this->db->join($this->mdl_company->table_name.' AS c','c.company_id = a.company_id','INNER');
		$this->db->join($this->mdl_parts->table_name.' AS pd','pd.part_number = a.part_number','INNER');
		$this->db->join($this->mdl_engineers->table_name.' AS e','e.engineer_id = a.engineer_id','INNER');
