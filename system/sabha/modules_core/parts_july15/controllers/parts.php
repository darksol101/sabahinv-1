...
class Parts extends Admin_Controller
{
    ...
    public function itemPrice($item=NULL)
    {   
        $item = 'SN10FW-DM001MX08';
        if($item){
            $item_price = $this->db->select('part_customer_price')
            ->from('sst_prod_parts')
            ->where('part_number',$item)
            ->get()->row();
    ...
    private function barcode($part_number, $part_id) {
        ...
        $barcodeOptions = array('text' => strtoupper($part_number), 'barHeight' => '67.9', 'factor' => 2,'font'=>5);
        
        // No required options
        $rendererOptions = array();
        Zend_Barcode_Object::setBarcodeFont('assets/free30f9/free3of9.ttf');
        ...
        //Zend_Barcode_Object::setBarcodeFont('system/fonts/Tahoma.ttf');

        $barcodeOptions = array(
            'text' => $part_number, 
            'barHeight'=> 74, 
            'factor'=>3.98,
            'font' => 'system/fonts/Tahoma.ttf',
        );
    ...
    function barcodes() {
        ...
        $barcodeOptions = array(
            ...
            'font' => 'assets/fonts/code_39_logitogo/code128.ttf',
        );
