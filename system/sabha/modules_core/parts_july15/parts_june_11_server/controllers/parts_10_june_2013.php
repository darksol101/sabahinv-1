...
class Parts extends Admin_Controller {
	...
	if ($partcheck == 0){
		if((int)$part_id==0){
			...
			$error = array('type'=>'failure','message'=>$this->lang->line('part_number_already'));
			}
		...
		function create_excel()
	{
		...
		$fields = array('Item Number','Landing Price','Customer Price','Store Price','Item description');
		$this->load->helper('download');
	...
	{
		$this->redir->set_last_index();
		$this->load->model(array('parts/mdl_parts_models','mcb_data/mdl_mcb_data','productmodel/mdl_productmodel','brands/mdl_brands','products/mdl_products'));
		$ajaxaction=$this->input->post('ajaxaction');
		$this->load->library('ajaxpagination');
		$config['base_url'] = base_url();
		$config['per_page'] = $this->mdl_mcb_data->get('per_page');
	...
	{
		ini_set("memory_limit","256M");
		$this->load->model(array('parts/mdl_parts_models','productmodel/mdl_productmodel','brands/mdl_brands','products/mdl_products'));
		$this->load->plugin('to_excel');
		$this->load->helper('calls');
		...
		$fields = array('Brand','Product','Model','Part');
		$this->load->helper('download');
		$data = convert_to_table($data,$fields);
