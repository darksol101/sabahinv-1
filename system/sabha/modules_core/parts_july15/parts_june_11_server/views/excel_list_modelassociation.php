...
$model_numbers = explode(',', str_replace(" ", "", $model_numbers['model_number']));
...
for ($i = 2; $i <= $row_count; $i++) {
	...
	if(in_array($rows[$i][2], $part_list)){
			$duplicate = true;
		}
		
		$no_models = false;

	if(!in_array($rows[$i][3], $model_numbers)){
		$no_models = true;
	}
    ...
    <tr<?php echo $trstyle;?>>
        ...
        <?php
			...
			if($no_models=="true"){
				echo '<span style="padding:0 1px;">Model Number is Not Registered.</span>';
				$selected = 3;
			}
		?> 
        </td>
