...
function appendImages(no_of,img) {
	if(no_of==0 || no_of==''){
		$(".bar_img").show();
	...
	else
	{
		...
		var style="width:10cm; height:2.24cm; padding:0 0.3cm 0 0";
		var style2="width:10cm; height:2.24cm; padding:0.2cm 0.3cm 0 0";
...
function appendImages_a4(no_of,img) {
	if(no_of==0 || no_of==''){
		$(".bar_img").show();
		$("#barcode_list_all").empty();
	}
...
function barcodes(part_number,printer) {

	$.facebox(function () {
		$.post('<?php echo site_url();?>parts/barcodes_print', {part_number:part_number,printer:printer,unq:ajaxunq()}, 
			function(data) {
				$.facebox(data); 
			});
	});
