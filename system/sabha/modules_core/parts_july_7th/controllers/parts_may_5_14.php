...
/*	dump($this->input->post());
	die();*/
		...
		if((int)$part_id==0){
			...
			$checkEdit=$this->db->select('part_number')
							->from($this->mdl_parts->table_name)
	...
	function getmodelsbyproducts(){
		...
		$data = array(
					  //'parts_assign_models'=>$parts_assign_models,
					  'part_assign_models_id'=>$part_assign_models_id,
					  'models'=>$models
		);
	...
	function savep()
		{
			...
			$this->load->model(array('parts/mdl_parts'));
			$part_initial_no = $this->input->post('part_no');
			$part_dsc = $this->input->post('part_dsc');
			$part_size = $this->input->post('part_size');
			$part_color = $this->input->post('part_color');
			$part_lp = $this->input->post('company');
			$part_cp = $this->input->post('quantity');
			$part_sp = $this->input->post('quantity_sc');
			...
			foreach($task_select as $task){
				if($task<3 && $task>=0){
					if($task==1){
						$part_no[$i]=$part_initial_no[$i];
						if($part_size){
							$part_no[$i] .= "-".$part_size[$i];	
							}
							if($part_color){
								$part_no[$i] .= "-".$part_color[$i];
							}
						...
						$data = array(
									  ...
									  'part_color'=>$part_color[$i],
									  'order_level'=>$order_level[$i],
...
function create_excel()
	{
		...
		$fields = array('Item Number','Part Color','Part Size','Order Level','Landing Price','Customer Price','Store Price','Item description');
		$this->load->helper('download');
	...
	function savep_modelassociation()
	{
			...
			foreach($task_select as $task){
				if($task<3 && $task>=0){
					...
					if($task==1){
						$data = array(
									  'part_id'=>$this->db->escape_like_str($part_no[$i]),
									  'model_id'=>$this->db->escape_like_str($model_no[$i])
									  );
			...
			foreach($task_select as $task){
				if($task<3 && $task>=0){
					...
					if($task==1){
						$data = array(
									  'part_number'=>$this->db->escape_like_str($part_no[$i]),
									  'model_number'=>$this->db->escape_like_str($model_no[$i]),
						...
						$data = array(
									  'model_number'=>$part_no[$i],
									  'part_last_mod_by'=>$this->session->userdata('user_id'),
									  'part_last_mod_ts'=>date("Y-m-d H:i:s")
									  );
						$this->db->where('part_number', $part_no[$i]);
