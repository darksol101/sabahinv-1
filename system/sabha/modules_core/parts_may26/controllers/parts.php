...
class Parts extends Admin_Controller
{
    ...
    private function barcode($part_number, $part_id) {
        ...
        $barcodeOptions = array('text' => $part_number, 'barHeight' => 20, 'factor' => 1.2);
        ...
        foreach ($task_select as $task) {
            if ($task < 3 && $task >= 0) {
                if ($task == 1) {
                    ...
                    $data = array('part_number' => $this->db->escape_like_str($part_number[$i]), 'part_initial_no' => $this->db->escape_like_str($part_initial_no[$i]), 'part_desc' => $this->db->escape_like_str($part_desc[$i] . "," . $remarks[$i]), 'part_landing_price' => $part_lp[$i], 'part_customer_price' => $part_cp[$i], 'part_sc_price' => $part_sp[$i]);
    ...
    function upload_modelassociation() {
        ...
        if (!$this->upload->do_upload($file_name)) {
            ...
            $data = array('rows' => $rows, 'row_count' => $row_count, 'parts' => $parts
