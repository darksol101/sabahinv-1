	...
	foreach ($parts as $part){
		...
		<tr <?php echo $trstyle;?>>
			...
			<td style="text-align: center;">
				<div>
						<img width="240" height="42" style="margin-top:10px;" src="<?php echo site_url().$barcodelink;?>" alt="<?php echo $part->part_desc;?>" onclick="print_barcode('barcode_<?php echo $part->part_id?>','<?php echo $part->part_number;?>','<?php echo $barcodelink;?>');">
					<p>MRP: <?php echo $part->part_customer_price;?></p>
				</div>
			
		
			
					<div id="barcode_<?php echo $part->part_id?>" class="barcode" style="display:none;">

						<button class="closePop" onclick="closePop('barcode_<?php echo $part->part_id?>')">X</button>
							<div style="float:left;" class="bar_img">
							<img src="<?php echo site_url().$barcodelink;?>" alt="<?php echo $part->part_desc;?>" class="bar_img">
							<p>MRP: <?php echo $part->part_customer_price;?></p>
							</div>
							<div id="<?php echo $part->part_number?>" class="barcode_list">
								
							</div>
						<br>
						<div class="takein">
						<input type="text" name="noof" id="print_no_of" value="1" onkeyup="appendImages(this.value,'<?php echo $barcodelink;?>','<?php echo $part->part_number?>','<?php echo $part->part_customer_price;?>')"/>
						<button onclick="print_no_barcodes('<?php echo $part->part_number?>');">Print</button>
						<button onclick="closePop('barcode_<?php echo $part->part_id?>')">Cancel</button>
						</div>
				</div>
		</td>
