...
function addpart(){
	...
	if(part_rate==''){
		$('#part_rate').validationEngine('showPrompt', '* This field is required', 'error', 'topRight', true);
		status = 0;
	}
	if(status==1){
		...
		html+='<td style="text-align: left;"><input type="hidden" name="prate[]" value="'+part_rate+'" class="text-input" /><span class="lbl">'+part_rate+'</span></td>';
		...
		if(purchase_details_id==0){
			...
			$("#rowdata tr").each(function(index) {
				 ...
				 if(ptn == part_number){
					 if(confirm('Item Number '+part_number+' already exists,Do you want to update it ?')){
						...
						$(this).find('td:first').next().next().next().find('input').val(part_rate);
						$(this).find('td:first').next().next().next().find('span.lbl').html(part_rate);
					 }
					 add = 0;
				 }
			});
			...
			ele.find('td:first').next().next().next().find('input').val(part_rate);
			ele.find('td:first').next().next().next().find('span.lbl').html(part_rate);
