...
<table style="width: 100%" class="tblgrid" cellpadding="0" cellspacing="0">
	<col width="1%" /><col width="10%"/><col width="10%" /><col width="20%" /><col width="20%"><col width="10%" /><col width="15%" /><col width="5%" />
	<thead>
		<tr>
	...
	foreach ($purchases as $purchase){
		...
		<tr <?php echo $trstyle;?>>
			...
			<td style="text-align:left;"><a><?php echo $purchase->purchase_id;?></a></td>
            ...
            <td><?php if($purchase->purchase_status<1){?><a class="btn"
				onclick="deletepurchase('<?php echo $purchase->purchase_id;?>')"><?php echo icon("delete","Delete","png");?></a><?php }else{?> <?php echo icon("delete_disable","Delete","png");?> <?php }?></td>
		</tr>
		<?php
		$i++;
	}
