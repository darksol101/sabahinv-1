...
class Callhistory extends Admin_Controller {
	function __construct() {
		parent::__construct();
		$this->load->language("callcenterhistory",  $this->mdl_mcb_data->setting('default_language'));
		$this->load->model('mdl_callhistory');
		$this->_post_handler();
	...
	function calls() {
		...
		$calls = $this->mdl_callhistory->getCalls($pg);
		...
		$this->load->view('callhistory/tab_calls',$data);
	}
	function create_excel()
	{
		ini_set("memory_limit","256M");
		$this->load->model(array('productmodel/mdl_productmodel','brands/mdl_brands','cities/mdl_cities','engineers/mdl_engineers','servicecenters/mdl_servicecenters','products/mdl_products','mcb_data/mdl_mcb_data','customers/mdl_customers','faultsettings/mdl_defect','faultsettings/mdl_repair','faultsettings/mdl_Symptom','productmodel/mdl_product_serial_number','zones/mdl_districts','zones/mdl_zones'));
		$this->load->model('users/mdl_users');
		$this->load->plugin('to_excel');
		$this->load->helper('calls');
		$pg['start'] = 0;
		$list = $this->mdl_callhistory->getCalls($pg);
		$result = $list['result'];
		$data = '';
		if(count($result->result())==0){
			redirect('callhistory/calls');
		}
		$call_id =array();
		...
		$fields = array('Call ID','Reg Date/Time','First Name','Last Name','Zone','District','City','Address','Office Phone','Home Phone','Mobile Number','Brand','Product','Model Number','Engineer','Aging Date/Time','Store',
	...
	function callregistration(){
	   ...
	   $call_details = $this->mdl_callhistory->getCallDetails($call_id);
	...
	function searchcall()
	{
		...
		redirect('reports/callhistory/calls'.$url);
		die();
	}
	function jobcard()
	{
		...
		$this->load->model(array('products/mdl_products',
						   ...
						   $preview_details = $this->mdl_callhistory->getCallPreviewDetails($call_id);
	...
	function changecallstatus(){
		...
		$this->mdl_callhistory->save($params,$call_id);
	...
	function _post_handler(){
		if($this->input->post('btn_save')){
			...
			$check_serial_number = $this->mdl_callhistory->checkopencallbyserial_number($call_id,$this->input->post('model_id'), $this->input->post('product_serial_number'));
			...
			if((int)$call_id==0){
				...
				$data_call['call_uid'] =$this->mdl_callhistory->generateCallIdByZone($zone_id);
				...
				if($this->mdl_callhistory->save($data_call)){
			...
			//print_r($defected_company);
				...
				if($this->input->post('cstatuslist')==3){
					...
					$row=$this->mdl_callhistory->get_by_id($call_id);
				...
				if($this->mdl_callhistory->save($data_call,$call_id)){
					$this->session->set_flashdata('save_status',true);
	...
	public function happycall ($callid){
		...
		$this->load->models(array('callcenter/mdl_call_happy','callcenter/mdl_call_happy_record','callcenter/mdl_call_happy_question','callcenter/mdl_calls_happy_answer','utilities/mdl_html'));
		
		$questions= $this->mdl_call_happy_question->getQuestions();
		$call_uid = $this->mdl_callhistory->getUidbyid($callid);
		...
		for($i=0; $i< $questions['total']; $i++)
		{
			...
			$happy_call_id= $this->mdl_call_happy->getid($ques[$i],$callid);
		     if ($happy_call_id >0)
				 ...
				 $caldata['call_last_mod_by']= $this->session->userdata('user_id');
                 $caldata['call_last_mod_ts']= date('Y-m-d H:i:s');
				 $caldata['happy_status']=1;
				$this->mdl_callhistory->save($caldata,$callid);
