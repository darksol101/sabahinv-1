...
class mdl_closedcallreports extends MY_Model {
	...
	*/
	public function getTotalClosedCallsByDate($sc_id){
		...
		if($product_id!='null'){
			$this->db->where($this->mdl_productmodel->table_name.'.product_id IN ('.$product_id.')');
		}
		if($call_type){
			$this->db->where("call_type IN(".$call_type.")");
		}
		if(!empty($from_date)){
		...
		$this->db->join($this->mdl_productmodel->table_name,$this->mdl_productmodel->table_name.'.model_id='.$this->table_name.'.model_id','left');
		$query = $this->db->get();
	...
	*/
	...
	public function getClosedCallsByTimeBetween2and7($sc_id)
	{
		$from_date = date("Y-m-d",date_to_timestamp($this->input->post('from_date')));
		$to_date = date("Y-m-d",date_to_timestamp($this->input->post('to_date')));
		$call_type = $this->input->post('call_type');
	...
	public function getClosedCallsByTimeBetween7and15($sc_id)
	{
		$from_date = date("Y-m-d",date_to_timestamp($this->input->post('from_date')));
		$to_date = date("Y-m-d",date_to_timestamp($this->input->post('to_date')));
		$call_type = $this->input->post('call_type');
	...
	public function getClosedCallsByTimeGreaterthan15($sc_id)
	{
		$from_date = date("Y-m-d",date_to_timestamp($this->input->post('from_date')));
		$to_date = date("Y-m-d",date_to_timestamp($this->input->post('to_date')));
		$call_type = $this->input->post('call_type');
