...
class Sales extends Admin_Controller{
	...
	function sale($sales_id = 0){
		...
		/*changes TEJ*/
		$sales_type = $this->mdl_mcb_data->getStatusOptions ('transaction_mode');
		array_unshift($sales_type, $this->mdl_html->option('','Select Sales Type'));
		$sales_type= $this->mdl_html->genericlist($sales_type,'sales_type',array('class'=>'validate[required] text-input'),'value','text','');
		...
		$this->form_validation->set_rules('service_center', 'Store', 'required');
...
public function getMakerDetail($sales_id,$part_id)
{	
	...
	$maker_details = $this->mdl_sales->getMakerDetail($sales_id,$part_id);
	return $maker_details;
...
function modsale(){
	...
	public function getSalesType($id=NULL)
	{
		/*change TEJ*/
		...
		$this->load->model('sales/mdl_sales');
		$dd = $this->mdl_sales->getVal('sales_type',$id);
		$sales_type = $this->mdl_mcb_data->getStatusOptions ('transaction_mode');
		foreach ($sales_type as $row) {
			if($row->value == $dd->sales_type){
				return $row->text;
			}
		}
