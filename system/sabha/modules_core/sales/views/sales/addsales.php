...
/*changes TEJ*/
    ...
    var sales_type = "<?php echo $salesdetails->sales_type; ?>"
    var bill_status  = "<?php echo $bill_details->bill_id; ?>"

    if(sales_type > 0){
      $("#sales_type option").each(function(){
           if(sales_type == $(this).val()){
...
<table width="100%">
  ...
  <?php if ($salesdetails->sales_id!= '') {?>
    ...
    <!--change TEJ-->
     <tr>
    <th><label><?php echo $this->lang->line('sales_type'); ?>: </label></th>
...
$(document).ready(function () {
  ...
  $("#received").blur(function (e) {
    ...
    if(rec!="" && rec > grand_total){

         var returns = (rec - grand_total).toFixed(2);
        $("#returns").val(returns);
    }
