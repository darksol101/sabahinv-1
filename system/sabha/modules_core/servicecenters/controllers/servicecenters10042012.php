...
class Servicecenters extends Admin_Controller {
	...
	function index() {
		...
		$cityOptions = $this->mdl_cities->getCityOptions('');
		array_unshift($cityOptions, $this->mdl_html->option( '', 'Select City'));
		$city_select  =  $this->mdl_html->genericlist($cityOptions,'city_select',array('class'=>'validate[required] select-one'),'value','text','');
		$data = array(
	...
	function tab_servicecenters() {
		$this->redir->set_last_index();	
		$this->load->model( array('cities/mdl_cities','utilities/mdl_html'));
		$cityOptions = $this->mdl_cities->getCityOptions(1);
		array_unshift($cityOptions, $this->mdl_html->option( '', 'Select City'));
		$city_select  =  $this->mdl_html->genericlist($cityOptions,'city_select',array('class'=>'validate[required] select-one'),'value','text','');
		$data = array(
					  'city_select'=>$city_select
