...
class mdl_Servicecenters extends MY_Model {
	...
	public function getServiceCenterList(){
		...
		$params=array(
					 "select"=>"sc_code, sc_id, sc_name, sc_address,sc_email,sc_fax,sc_address",
					 "order_by"=>"sc_name"
					 );
		if(!empty($searchtxt)){
	...
	public function getServiceCenterdetails($id){
		$params=array(
					 "select"=>"sc_code, sc_id, sc_name, sc_address,  sc_phone1, sc_phone2, sc_phone3, sc_fax, sc_email,city_id",
	...
	function getServiceCentersOptionsBySc($sc_id)
	{
		$params=array(
					 "select"=>"sc_id as value, sc_name as text",
					 "order_by"=>'text'
					 );
		$params["where"] = array('sc_id IN('.$sc_id.')');
		$servicecenters=$this->get($params);
		return $servicecenters;
	}
...
function getscdetailsbyid($id)
	{
		$this->db->select('sc_name,sc_address,sc_phone1,sc_fax');
	...
	function getServiceCenterdetails_alloc($id){
		$this->db->select('sc.*');
		$this->db->from($this->table_name.' AS sc');
		...
		function getScCode($sc_id){
		$this->db->select('sc.sc_code AS sc_code');
		$this->db->from($this->table_name.' AS sc');
