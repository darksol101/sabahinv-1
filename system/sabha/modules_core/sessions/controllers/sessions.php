    ...
    function login() {
		...
		if ($this->mdl_sessions->validate()) {
			...
			if ($user = $this->mdl_auth->auth('mcb_users', 'username', 'password', $this->input->post('username'), $this->input->post('password'))) {
				...
				/*if($user->ustatus!=1){
					redirect('sessions/login',$this->session->set_flashdata('login_error',$this->lang->line('user_blocked')));
				}*/

				$this->load->model(array('products/mdl_products','users/mdl_user_product_assigns'));
				$user_product = $this->mdl_user_product_assigns->getProductsByUser($user->user_id);
				$arr = array();
				foreach($user_product as $row){
					$arr[]=$row->product_id;
				}
				$user_products = '0';
				if(count($arr)>0){
					$user_products = implode(",",$arr);
				}
				...
				$this->mdl_auth->set_session($user, $object_vars, array('is_admin'=>TRUE, 'is_loggedin'=>TRUE,'sc_id'=>$sc_id,'sc_name'=>$sc_name,'city_id'=>$city_id,'city_name'=>$city_name,'zone_name'=>$zone_name,'zone_code'=>$zone_code,'district_name'=>$district_name,'username'=>$username,'user_products'=>$user_products));
