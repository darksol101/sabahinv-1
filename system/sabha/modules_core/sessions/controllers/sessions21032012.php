    ...
    function login() {
		...
		if ($this->mdl_sessions->validate()) {
			...
			if ($user = $this->mdl_auth->auth('mcb_users', 'username', 'password', $this->input->post('username'), $this->input->post('password'))) {
				$object_vars = array('user_id', 'last_name', 'first_name', 'global_admin', 'userid');
				...
				redirect('sessions/login',$this->session->set_flashdata('custom_error',$this->lang->line('invalid_username_or_password')));
			}

		}
		$data=array("title"=>"Log in", "headline"=>"User Log in",'sc_name'=>$sc_name);
